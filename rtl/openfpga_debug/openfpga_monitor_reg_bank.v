...
always @(*) begin
    ...
    case (req_addr)
        `OFD_MON_ADDR_ID: old_value = `OFD_MONITOR_ID_VALUE;
        `OFD_MON_ADDR_VERSION: old_value = `OFD_MONITOR_VERSION_VALUE;
        `OFD_MON_ADDR_CONTROL: begin
            old_value = control;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LED_CONTROL: begin
            old_value = led_control;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_DEMO_PERIOD: begin
            old_value = demo_period;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_COUNTER0: old_value = counter0;
        `OFD_MON_ADDR_CLEAR_COUNTERS: begin
            old_value = 32'd0;
            is_writable = 1'b1;
            is_trigger = 1'b1;
        end
        `OFD_MON_ADDR_ERROR_STATUS: begin
            old_value = error_status;
            is_writable = 1'b1;
            is_w1c = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_ID: old_value = `OFD_PROFILER_ID_VALUE;
        `OFD_MON_ADDR_PROFILER_VERSION: old_value = `OFD_PROFILER_VERSION_VALUE;
        `OFD_MON_ADDR_PROFILER_CONTROL: begin
            old_value = profiler_control;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD: begin
            old_value = profiler_sample_period;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_CLEAR: begin
            old_value = 32'd0;
            is_writable = 1'b1;
            is_trigger = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_STATUS: begin
            old_value = profiler_status | profiler_status_set;
            is_writable = 1'b1;
            is_w1c = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_METRIC_MASK0: begin
            old_value = profiler_metric_mask0;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_PROFILER_ALERT_THRESHOLD0: begin
            old_value = profiler_alert_threshold0;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_ID: old_value = `OFD_LA_ID_VALUE;
        `OFD_MON_ADDR_LA_VERSION: old_value = `OFD_LA_VERSION_VALUE;
        `OFD_MON_ADDR_LA_CONTROL: begin
        ...
        `OFD_MON_ADDR_LA_SAMPLE_DIVISOR: begin
            old_value = la_sample_divisor;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_CAPTURE_DEPTH: begin
            old_value = la_capture_depth;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_PRETRIGGER_DEPTH: begin
            old_value = la_pretrigger_depth;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_TRIGGER_MODE: begin
            old_value = la_trigger_mode;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_TRIGGER_CHANNEL: begin
            old_value = la_trigger_channel;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_TRIGGER_VALUE: begin
            old_value = la_trigger_value;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_TRIGGER_MASK: begin
            old_value = la_trigger_mask;
            is_writable = 1'b1;
        end
        `OFD_MON_ADDR_LA_COMMAND: begin
            old_value = 32'd0;
            is_writable = 1'b1;
            is_trigger = 1'b1;
        end
        `OFD_MON_ADDR_LA_CAPTURE_ID: old_value = la_capture_id;
        `OFD_MON_ADDR_LA_CHANNEL_MASK: begin
    ...
    new_value = old_value;
    status = `OFD_MON_STATUS_OK;

    if (!is_known) begin
        status = `OFD_MON_STATUS_BAD_ADDR;
    end else if (req_width != 8'd4) begin
        status = `OFD_MON_STATUS_BAD_LEN;
    end else if (req_write && !is_writable) begin
        status = `OFD_MON_STATUS_DENIED;
    end else if (req_write && req_addr == `OFD_MON_ADDR_DEMO_PERIOD && ((old_value & ~req_wmask) | (req_wdata & req_wmask)) == 32'd0) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD && ((old_value & ~req_wmask) | (req_wdata & req_wmask)) == 32'd0) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_LA_SAMPLE_DIVISOR && ((old_value & ~req_wmask) | (req_wdata & req_wmask)) == 32'd0) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_LA_CAPTURE_DEPTH &&
                 ((((old_value & ~req_wmask) | (req_wdata & req_wmask)) == 32'd0) ||
                  (((old_value & ~req_wmask) | (req_wdata & req_wmask)) > {16'd0, `OFD_LA_MAX_SAMPLE_DEPTH}))) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_LA_PRETRIGGER_DEPTH &&
                 (((old_value & ~req_wmask) | (req_wdata & req_wmask)) >= la_capture_depth)) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_LA_TRIGGER_MODE &&
                 (((old_value & ~req_wmask) | (req_wdata & req_wmask)) > {28'd0, `OFD_LA_TRIGGER_MASK_MATCH})) begin
        status = `OFD_MON_STATUS_BAD_VALUE;
    end else if (req_write && req_addr == `OFD_MON_ADDR_LA_TRIGGER_CHANNEL &&
...
always @(posedge clk) begin
    if (rst) begin
        ...
        profiler_sample_period <= `OFD_PROFILER_DEFAULT_SAMPLE_PERIOD;
        ...
        la_trigger_mode <= {28'd0, `OFD_LA_TRIGGER_MASK_MATCH};
        ...
        if (req_valid && req_ready) begin
            ...
            resp_new_value <= (status == `OFD_MON_STATUS_OK) ? new_value : old_value;

            if (status == `OFD_MON_STATUS_OK && req_write) begin
                case (req_addr)
                    `OFD_MON_ADDR_CONTROL: control <= new_value;
                    `OFD_MON_ADDR_LED_CONTROL: led_control <= new_value;
                    `OFD_MON_ADDR_DEMO_PERIOD: demo_period <= new_value;
                    `OFD_MON_ADDR_CLEAR_COUNTERS: clear_counters_pulse <= |req_wdata;
                    `OFD_MON_ADDR_ERROR_STATUS: error_status <= new_value;
                    `OFD_MON_ADDR_PROFILER_CONTROL: profiler_control <= new_value;
                    `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD: profiler_sample_period <= new_value;
                    `OFD_MON_ADDR_PROFILER_CLEAR: profiler_clear_pulse <= |req_wdata;
                    `OFD_MON_ADDR_PROFILER_STATUS: profiler_status <= new_value | profiler_status_set;
                    ...
                    `OFD_MON_ADDR_LA_TRIGGER_MODE: la_trigger_mode <= new_value;
                    `OFD_MON_ADDR_LA_TRIGGER_CHANNEL: la_trigger_channel <= new_value;
                    `OFD_MON_ADDR_LA_TRIGGER_VALUE: la_trigger_value <= new_value;
                    `OFD_MON_ADDR_LA_TRIGGER_MASK: la_trigger_mask <= new_value;
                    `OFD_MON_ADDR_LA_COMMAND: begin
                    ...
                    `OFD_MON_ADDR_LA_CHANNEL_MASK: la_channel_mask <= new_value;
                    default: begin end
                endcase
            end
        end
...
module openfpga_monitor_reg_bank #(
    parameter DEFAULT_DEMO_PERIOD = 32'd1000000
) (
    input  wire        clk,
    input  wire        rst,
    ...
    output wire  [31:0] resp_old_value,
    output wire  [31:0] resp_new_value,
