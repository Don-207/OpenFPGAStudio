...
module openfpga_debug_board_demo #(
    ...
    parameter WATCH_INTERVAL_TICKS = CLK_FREQ_HZ / 100,
    parameter PRINT_INTERVAL_TICKS = CLK_FREQ_HZ / 5,
    parameter STATUS_INTERVAL_TICKS = CLK_FREQ_HZ / 50,
...
wire la_readout_busy = la_core_state == `OFD_LA_STATE_READOUT;
...
assign profiler_metric_id =
    (profiler_metric_select == 2'd0) ? `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT :
    (profiler_metric_select == 2'd1) ? `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL :
    (profiler_metric_select == 2'd2) ? `OFD_PROFILER_METRIC_DEMO_LATENCY :
                                       `OFD_PROFILER_METRIC_FRAME_RATE;
...
openfpga_la_core #(
    ...
    .trigger_mode(la_control[2] ? la_trigger_mode[3:0] : `OFD_LA_TRIGGER_DISABLED),
...
openfpga_debug_top #(
    ...
    .trace_drop_trace_id(`OFD_TRACE_ID_GLOBAL),
