...
wire active_capture = (state == `OFD_LA_STATE_ARMED) || (state == `OFD_LA_STATE_CAPTURING);
wire trigger_hit;
...
task finish_capture;
    input forced;
    begin
        state <= `OFD_LA_STATE_DONE;
        read_start_ptr <= calc_start_ptr(write_ptr, samples_written);
        capture_flags <= `OFD_LA_FLAG_VALID |
                         `OFD_LA_FLAG_TRIGGERED |
                         (forced ? `OFD_LA_FLAG_FORCED : 16'd0) |
                         (overflow ? `OFD_LA_FLAG_OVERFLOW : 16'd0) |
                         ((samples_written < effective_capture_depth) ? `OFD_LA_FLAG_PARTIAL : 16'd0);
    end
endtask

always @(posedge clk) begin
    if (rst) begin
        state <= `OFD_LA_STATE_IDLE;
        samples_written <= 16'd0;
        trigger_index <= 16'd0;
        capture_id <= 32'd0;
        overflow <= 1'b0;
        config_error <= 1'b0;
        error_code <= `OFD_LA_ERROR_NONE;
        capture_flags <= 16'd0;
        ...
        if (clear_pulse || !enable) begin
            ...
            if (read_req && ((state == `OFD_LA_STATE_DONE) || (state == `OFD_LA_STATE_READOUT)) &&
            ...
            case (state)
                `OFD_LA_STATE_IDLE: begin
                    if (arm_pulse) begin
                        if (config_bad) begin
                            state <= `OFD_LA_STATE_ERROR;
                            config_error <= 1'b1;
                            error_code <= `OFD_LA_ERROR_CONFIG;
                        end else begin
                            state <= `OFD_LA_STATE_ARMED;
                ...
                `OFD_LA_STATE_ARMED: begin
                    if (stop_pulse) begin
                        ...
                        if (samples_written >= effective_capture_depth) begin
                            ...
                            state <= `OFD_LA_STATE_CAPTURING;
                            post_remaining <= effective_capture_depth - samples_written;
                            capture_flags <= `OFD_LA_FLAG_FORCED;
                        end
                ...
                `OFD_LA_STATE_CAPTURING: begin
                    if (stop_pulse) begin
                        finish_capture((capture_flags & `OFD_LA_FLAG_FORCED) != 16'd0);
                        ...
                        if (post_remaining == 16'd1) begin
                            ...
                            capture_flags <= `OFD_LA_FLAG_VALID |
                                             `OFD_LA_FLAG_TRIGGERED |
                                             (((capture_flags & `OFD_LA_FLAG_FORCED) != 16'd0) ? `OFD_LA_FLAG_FORCED : 16'd0) |
                                             (overflow ? `OFD_LA_FLAG_OVERFLOW : 16'd0);
                        end
                    end
                end

                `OFD_LA_STATE_DONE: begin
                    if (start_readout_pulse) begin
                        state <= `OFD_LA_STATE_READOUT;
                ...
                `OFD_LA_STATE_READOUT: begin
                    if (readout_done_pulse) begin
                        state <= `OFD_LA_STATE_DONE;
                    end
                end

                `OFD_LA_STATE_ERROR: begin
