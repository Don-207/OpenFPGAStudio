...
wire [15:0] next_samples_in_chunk =
    (remaining_samples > {8'd0, `OFD_LA_SAMPLES_PER_CHUNK}) ? {8'd0, `OFD_LA_SAMPLES_PER_CHUNK} : remaining_samples;
wire capture_has_trigger = (core_capture_flags & `OFD_LA_FLAG_TRIGGERED) != 16'd0;
...
always @(posedge clk) begin
    if (rst) begin
        ...
        case (state)
            ST_IDLE: begin
                if (start_readout_pulse && (core_state == `OFD_LA_STATE_DONE)) begin
                    core_start_readout_pulse <= 1'b1;
                    chunks_total <= (core_samples_written + {8'd0, `OFD_LA_SAMPLES_PER_CHUNK} - 16'd1) /
                                    {8'd0, `OFD_LA_SAMPLES_PER_CHUNK};
                    chunk_index <= 16'd0;
                    sample_index <= 16'd0;
                    chunks_sent <= 16'd0;
                    state <= ST_HEADER;
            ...
            ST_HEADER: begin
                if (can_load_msg) begin
                    msg_valid <= 1'b1;
                    msg_type <= `OFD_TYPE_LA_CAPTURE_HEADER;
                    payload_len <= `OFD_LA_LEN_CAPTURE_HEADER;
                    payload_flat <= 256'd0;
                    payload_flat[31:0] <= core_capture_id;
                    payload_flat[63:32] <= timestamp;
                    payload_flat[79:64] <= `OFD_LA_SAMPLE_WIDTH_BITS;
                    ...
                    payload_flat[175:160] <= `OFD_LA_CHANNEL_COUNT;
            ...
            ST_TRIGGER: begin
                if (can_load_msg) begin
                    msg_valid <= 1'b1;
                    msg_type <= `OFD_TYPE_LA_TRIGGER_EVENT;
                    payload_len <= `OFD_LA_LEN_TRIGGER_EVENT;
                    payload_flat <= 256'd0;
            ...
            ST_READ: begin
                if (sample_index >= core_samples_written) begin
                    ...
                    data_payload[71:64] <= `OFD_LA_SAMPLE_BYTES;
            ...
            ST_DATA: begin
                if (core_read_valid) begin
                    ...
                    if ((core_read_index - sample_index + 16'd1) >= samples_in_chunk) begin
                        if (can_load_msg) begin
                            msg_valid <= 1'b1;
                            msg_type <= `OFD_TYPE_LA_SAMPLE_DATA;
                            payload_len <= `OFD_LA_LEN_SAMPLE_DATA;
            ...
            ST_STATUS: begin
                if (can_load_msg) begin
                    msg_valid <= 1'b1;
                    msg_type <= `OFD_TYPE_LA_CAPTURE_STATUS;
                    payload_len <= `OFD_LA_LEN_CAPTURE_STATUS;
                    payload_flat <= 256'd0;
                    payload_flat[31:0] <= timestamp;
                    payload_flat[63:32] <= core_capture_id;
