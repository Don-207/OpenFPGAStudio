...
wire is_read_req = frame_type == `OFD_TYPE_MONITOR_READ_REQ;
wire is_write_req = frame_type == `OFD_TYPE_MONITOR_WRITE_REQ;
    ...
    if (rst) begin
        ...
        if (byte_valid && !pending_block) begin
            case (state)
                ...
                STATE_VERSION: begin
                    if (byte_data == `OFD_VERSION) begin
                        checksum <= byte_data;
                        state <= STATE_TYPE;
                    end else if (byte_data == `OFD_SOF) begin
                        state <= STATE_VERSION;
                        checksum <= 8'd0;
                    end else begin
                        state <= STATE_SOF;
                ...
                STATE_LEN: begin
                    ...
                    payload_index <= 8'd0;
                    checksum <= checksum ^ byte_data;
                    if (byte_data > `OFD_MAX_PAYLOAD_BYTES) begin
                        bad_len_error <= 1'b1;
                        state <= STATE_SOF;
