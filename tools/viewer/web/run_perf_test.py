...
def find_browser(explicit=None):
    candidate = explicit or os.environ.get("OPENFPGA_VIEWER_BROWSER")
    if candidate:
    ...
    raise RuntimeError(
        ...
        "or set OPENFPGA_VIEWER_BROWSER to its executable path."
    )
...
def main():
    ...
    profile = tempfile.mkdtemp(prefix="openfpga-viewer-cdp-")
    ...
    try:
        ...
        while time.time() < deadline:
            evaluated = cdp_call(sock, counter, "Runtime.evaluate", {
                "expression": "!!window.openfpgaViewerTest",
                "returnByValue": True,
            })
