...
module tb_openfpga_debug_board_profiler;
...
always @(posedge clk_p) begin
    if (dut.profiler_snapshot_valid && dut.profiler_snapshot_ready) begin
        ...
        if (dut.profiler_snapshot_metric_id == `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT) saw_axis = 1'b1;
        if (dut.profiler_snapshot_metric_id == `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL) saw_fifo = 1'b1;
        if (dut.profiler_snapshot_metric_id == `OFD_PROFILER_METRIC_DEMO_LATENCY) saw_latency = 1'b1;
        if (dut.profiler_snapshot_metric_id == `OFD_PROFILER_METRIC_FRAME_RATE) saw_frame = 1'b1;
    end
...
task read_frame;
    ...
    begin
        ...
        check(rx_byte == `OFD_SOF, "frame SOF mismatch");
        uart_read_byte(rx_byte);
        check(rx_byte == `OFD_VERSION, "frame version mismatch");
...
task send_monitor_write;
    ...
    reg [7:0] sum;
    begin
        sum = `OFD_VERSION ^ `OFD_TYPE_MONITOR_WRITE_REQ ^ 8'd13 ^
        ...
        uart_write_byte(`OFD_SOF);
        uart_write_byte(`OFD_VERSION);
        uart_write_byte(`OFD_TYPE_MONITOR_WRITE_REQ);
...
task send_monitor_read;
    ...
    reg [7:0] sum;
    begin
        sum = `OFD_VERSION ^ `OFD_TYPE_MONITOR_READ_REQ ^ 8'd5 ^
              seq[7:0] ^ seq[15:8] ^ addr[7:0] ^ addr[15:8] ^ 8'd4;
        uart_write_byte(`OFD_SOF);
        uart_write_byte(`OFD_VERSION);
        uart_write_byte(`OFD_TYPE_MONITOR_READ_REQ);
...
task read_until_activity;
    ...
    begin
        for (i = 0; i < max_frames; i = i + 1) begin
            ...
            if (frame_type == `OFD_TYPE_MONITOR_READ_RESP || frame_type == `OFD_TYPE_MONITOR_WRITE_RESP) begin
                saw_monitor = 1'b1;
            end else if (frame_type >= 8'h10 && frame_type <= 8'h14) begin
                saw_trace = 1'b1;
            end else if (frame_type == `OFD_TYPE_PROFILER_SNAPSHOT) begin
                snapshot_count = snapshot_count + 1;
                check(frame_len == `OFD_PROFILER_LEN_SNAPSHOT, "profiler snapshot length mismatch");
                metric_id = payload_u16(4);
                if (metric_id == `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT) begin
                    saw_axis = 1'b1;
                    saw_masked_axis = 1'b1;
                end
                if (metric_id == `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL) saw_fifo = 1'b1;
                if (metric_id == `OFD_PROFILER_METRIC_DEMO_LATENCY) saw_latency = 1'b1;
                if (metric_id == `OFD_PROFILER_METRIC_FRAME_RATE) saw_frame = 1'b1;
                check(payload_u32(8) != 32'd0, "profiler sample window should be nonzero");
            end else if (frame_type == `OFD_TYPE_PROFILER_ALERT) begin
                alert_count = alert_count + 1;
                check(frame_len == `OFD_PROFILER_LEN_ALERT, "profiler alert length mismatch");
            end
        end
    end
endtask
...
initial begin
    ...
    fork
        send_monitor_read(16'h2101, `OFD_MON_ADDR_PROFILER_ID);
        read_until_monitor_response(16'h2101, `OFD_TYPE_MONITOR_READ_RESP);
    join
    check(payload_u16(4) == 16'h2101, "PROFILER_ID response seq mismatch");
    check(payload[8] == `OFD_MON_STATUS_OK, "PROFILER_ID read should be OK");
    check(payload_u32(10) == `OFD_PROFILER_ID_VALUE, "PROFILER_ID value mismatch");

    fork
        send_monitor_write(16'h2103, `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD, 32'd8, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2103, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    check(payload[8] == `OFD_MON_STATUS_OK, "PROFILER_SAMPLE_PERIOD write should be OK");

    fork
        send_monitor_write(16'h2104, `OFD_MON_ADDR_PROFILER_ALERT_THRESHOLD0, 32'd16, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2104, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    check(payload[8] == `OFD_MON_STATUS_OK, "PROFILER_ALERT_THRESHOLD0 write should be OK");

    fork
        send_monitor_write(16'h2105, `OFD_MON_ADDR_PROFILER_CONTROL, 32'd1, 32'h00000001);
        read_until_monitor_response(16'h2105, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    check(payload[8] == `OFD_MON_STATUS_OK, "PROFILER_CONTROL enable should be OK");
    ...
    if (errors == 0) begin
        $display("PASS: OpenFPGA Profiler M21 board demo checks passed");
    end else begin
