...
module tb_openfpga_debug_board_monitor;
...
initial begin
    ...
    fork
        send_monitor_read(16'h1001, `OFD_MON_ADDR_ID);
        read_frame(frame_type, frame_len);
    join
    check(frame_type == `OFD_TYPE_MONITOR_READ_RESP, "MONITOR_ID should return read response");
    check(frame_len == 8'd14, "read response length mismatch");
    check(payload_u16(4) == 16'h1001, "read response seq mismatch");
    check(payload[8] == `OFD_MON_STATUS_OK, "MONITOR_ID read should be OK");
    check(payload_u32(10) == `OFD_MONITOR_ID_VALUE, "MONITOR_ID response value mismatch");

    fork
        send_monitor_write(16'h1002, `OFD_MON_ADDR_LED_CONTROL, 32'h00000003, 32'hFFFFFFFF);
        read_frame(frame_type, frame_len);
    join
    check(frame_type == `OFD_TYPE_MONITOR_WRITE_RESP, "LED_CONTROL should return write response");
    check(frame_len == 8'd17, "write response length mismatch");
    check(payload_u16(4) == 16'h1002, "write response seq mismatch");
    check(payload[8] == `OFD_MON_STATUS_OK, "LED_CONTROL write should be OK");
    repeat (5) @(posedge clk_p);
    check(led0 && led1, "LED_CONTROL should drive both board LEDs");

    fork
        send_monitor_write(16'h1003, `OFD_MON_ADDR_ID, 32'hFFFFFFFF, 32'hFFFFFFFF);
        read_frame(frame_type, frame_len);
    join
    check(frame_type == `OFD_TYPE_MONITOR_WRITE_RESP, "RO write should return write response");
    check(payload[8] == `OFD_MON_STATUS_DENIED, "RO write should be denied");

    if (errors == 0) begin
        $display("PASS: OpenFPGA Monitor M16 board UART RX to response path checks passed");
    end else begin
