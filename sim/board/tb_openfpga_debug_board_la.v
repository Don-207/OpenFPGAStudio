...
module tb_openfpga_debug_board_la;
...
task read_frame;
    ...
    begin
        ...
        while (rx_byte != `OFD_SOF && sync_count < 64) begin
...
task read_until_la_readout;
    ...
    begin
        for (i = 0; i < 120 && la_status_count == 0; i = i + 1) begin
            read_frame(frame_type, frame_len);
            if (frame_type == `OFD_TYPE_LA_CAPTURE_HEADER) begin
                la_header_count = la_header_count + 1;
                check(frame_len == `OFD_LA_LEN_CAPTURE_HEADER, "LA header length mismatch");
                check(payload_u16(8) == 16'd32, "LA sample width mismatch");
                check(payload_u16(10) != 16'd0, "LA sample count should be nonzero");
            end else if (frame_type == `OFD_TYPE_LA_TRIGGER_EVENT) begin
                la_trigger_count = la_trigger_count + 1;
                check(frame_len == `OFD_LA_LEN_TRIGGER_EVENT, "LA trigger length mismatch");
            end else if (frame_type == `OFD_TYPE_LA_SAMPLE_DATA) begin
                la_data_count = la_data_count + 1;
                check(frame_len == `OFD_LA_LEN_SAMPLE_DATA, "LA data length mismatch");
                check(payload[9] != 8'd0, "LA data chunk should contain samples");
            end else if (frame_type == `OFD_TYPE_LA_CAPTURE_STATUS) begin
                la_status_count = la_status_count + 1;
                check(frame_len == `OFD_LA_LEN_CAPTURE_STATUS, "LA status length mismatch");
                check(payload[8] == `OFD_LA_STATE_READOUT || payload[8] == `OFD_LA_STATE_DONE, "LA status state mismatch");
            end else if (frame_type == `OFD_TYPE_MONITOR_READ_RESP || frame_type == `OFD_TYPE_MONITOR_WRITE_RESP) begin
                monitor_count = monitor_count + 1;
            end else if (frame_type >= 8'h10 && frame_type <= 8'h14) begin
                saw_trace = 1'b1;
            end else if (frame_type >= 8'h30 && frame_type <= 8'h31) begin
...
initial begin
    ...
    fork
        send_monitor_read(16'h2501, `OFD_MON_ADDR_LA_ID);
        read_until_monitor_response(16'h2501, `OFD_TYPE_MONITOR_READ_RESP);
    join
    check(payload[8] == `OFD_MON_STATUS_OK, "LA_ID read should be OK");
    check(payload_u32(10) == `OFD_LA_ID_VALUE, "LA_ID value mismatch");

    fork
        send_monitor_write(16'h2503, `OFD_MON_ADDR_LA_CAPTURE_DEPTH, 32'd8, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2503, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    fork
    ...
    join
    fork
        send_monitor_write(16'h2506, `OFD_MON_ADDR_LA_COMMAND, 32'h00000001, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2506, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    ...
    fork
        send_monitor_write(16'h2508, `OFD_MON_ADDR_LA_COMMAND, 32'h00000010, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2508, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    ...
    fork
        send_monitor_write(16'h2509, `OFD_MON_ADDR_LA_COMMAND, 32'h00000004, 32'hFFFFFFFF);
        read_until_monitor_response(16'h2509, `OFD_TYPE_MONITOR_WRITE_RESP);
    join
    repeat (10) @(posedge clk_p);
    check(dut.la_core_state == `OFD_LA_STATE_IDLE, "LA clear should return core to IDLE");

    if (errors == 0) begin
        $display("PASS: OpenFPGA Logic Analyzer M25 board demo checks passed");
    end else begin
