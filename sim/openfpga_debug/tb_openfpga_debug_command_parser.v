...
module tb_openfpga_debug_command_parser;
...
openfpga_debug_command_parser u_parser (
    .clk(clk),
    .rst(rst),
...
initial begin
    ...
    fork
        send_read_req(16'h1234, `OFD_MON_ADDR_LED_CONTROL);
    ...
    check(monitor_req_addr == `OFD_MON_ADDR_LED_CONTROL, "read addr mismatch");
    check(monitor_req_width == 8'd4, "read width mismatch");
    @(posedge clk);

    fork
        send_write_req(16'h2345, `OFD_MON_ADDR_LED_CONTROL, 32'h00000005, 32'h0000000F, 1'b0);
    ...
    send_write_req(16'h3456, `OFD_MON_ADDR_LED_CONTROL, 32'h0000000A, 32'h0000000F, 1'b1);
    ...
    if (errors == 0) begin
        $display("PASS: OpenFPGA Monitor M13 UART RX command parser checks passed");
    end else begin
