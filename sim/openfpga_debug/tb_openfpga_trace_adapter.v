...
module tb_openfpga_trace_adapter;
...
initial begin
    ...
    check(msg_type == `OFD_TYPE_TRACE_SPAN_BEGIN, "span begin type mismatch");
    check(payload_len == `OFD_TRACE_LEN_SPAN_BEGIN, "span begin length mismatch");
    check(payload_flat[31:0] == 32'h00000064, "span begin timestamp mismatch");
    check(payload_flat[47:32] == `OFD_TRACE_ID_DMA, "span begin trace_id mismatch");
    ...
    span_end_valid = 1'b1;
    #1;
    check(msg_type == `OFD_TYPE_TRACE_SPAN_END, "span end type mismatch");
    check(payload_len == `OFD_TRACE_LEN_SPAN_END, "span end length mismatch");
    check(payload_flat[71:64] == `OFD_TRACE_STATUS_TIMEOUT, "span end status mismatch");
    ...
    mark_valid = 1'b1;
    #1;
    check(msg_type == `OFD_TYPE_TRACE_MARK, "mark type mismatch");
    check(payload_len == `OFD_TRACE_LEN_MARK, "mark length mismatch");
    ...
    value_valid = 1'b1;
    #1;
    check(msg_type == `OFD_TYPE_TRACE_VALUE, "value type mismatch");
    check(payload_len == `OFD_TRACE_LEN_VALUE, "value length mismatch");
    ...
    drop_trace_id = `OFD_TRACE_ID_GLOBAL;
    drop_count = 32'd3;
    drop_valid = 1'b1;
    #1;
    check(msg_type == `OFD_TYPE_TRACE_DROP, "drop type mismatch");
    check(payload_len == `OFD_TRACE_LEN_DROP, "drop length mismatch");
    check(payload_flat[47:32] == `OFD_TRACE_ID_GLOBAL, "drop trace id mismatch");
    ...
    mark_valid = 1'b1;
    #1;
    check(msg_type == `OFD_TYPE_TRACE_SPAN_BEGIN, "priority should select span begin before mark");
    ...
    if (errors == 0) begin
        $display("PASS: OpenFPGA Trace Adapter payload and handshake checks passed");
    end else begin
