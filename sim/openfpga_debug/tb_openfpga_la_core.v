...
reg [3:0] trigger_mode = `OFD_LA_TRIGGER_MASK_MATCH;
...
initial begin
    ...
    check(state == `OFD_LA_STATE_IDLE, "disabled core should ignore arm");

    enable = 1'b1;
    pulse_arm();
    check(state == `OFD_LA_STATE_ARMED, "arm should enter ARMED");
    drive_sample(32'd1);
    drive_sample(32'd2);
    drive_sample(32'd3);
    drive_sample(32'd4);
    ...
    check((capture_flags & `OFD_LA_FLAG_TRIGGERED) != 16'd0, "triggered flag missing");
    ...
    wait_msg(`OFD_TYPE_LA_CAPTURE_HEADER);
    check(payload_len == `OFD_LA_LEN_CAPTURE_HEADER, "header length mismatch");
    ...
    accept_msg();
    wait_msg(`OFD_TYPE_LA_TRIGGER_EVENT);
    check(payload_len == `OFD_LA_LEN_TRIGGER_EVENT, "trigger event length mismatch");
    check(payload_flat[127:96] == 32'd8, "trigger event sample mismatch");
    accept_msg();
    wait_msg(`OFD_TYPE_LA_SAMPLE_DATA);
    check(payload_len == `OFD_LA_LEN_SAMPLE_DATA, "sample data length mismatch");
    ...
    accept_msg();
    wait_msg(`OFD_TYPE_LA_CAPTURE_STATUS);
    check(payload_len == `OFD_LA_LEN_CAPTURE_STATUS, "status length mismatch");
    check(payload_flat[111:96] == 16'd2, "status chunks sent mismatch");
    accept_msg();
    repeat (2) @(posedge clk);
    check(state == `OFD_LA_STATE_DONE, "core should return to DONE after readout");
    ...
    check(msg_type == `OFD_TYPE_LA_CAPTURE_HEADER, "held message type mismatch");
    msg_ready = 1'b1;
    wait_msg(`OFD_TYPE_LA_CAPTURE_HEADER);
    accept_msg();
    while (msg_valid || state == `OFD_LA_STATE_READOUT) begin
        @(posedge clk);
        #1;
    end
    ...
    pulse_clear();
    check(state == `OFD_LA_STATE_IDLE, "clear should return to IDLE");
    check(samples_written == 16'd0, "clear should reset sample count");

    trigger_mode = `OFD_LA_TRIGGER_EDGE_RISING;
    ...
    pulse_clear();
    trigger_mode = `OFD_LA_TRIGGER_DISABLED;
    capture_depth = 16'd4;
    pretrigger_depth = 16'd2;
    pulse_arm();
    drive_sample(32'd11);
    ...
    drive_sample(32'd14);
    wait_done();
    check((capture_flags & `OFD_LA_FLAG_FORCED) != 16'd0, "forced flag missing");
    ...
    pulse_arm();
    check(state == `OFD_LA_STATE_ERROR, "bad depth should enter ERROR");
    check(config_error, "config error flag missing");
    check(error_code == `OFD_LA_ERROR_CONFIG, "config error code mismatch");

    if (errors == 0) begin
        $display("PASS: OpenFPGA Logic Analyzer M23 core capture checks passed");
    end else begin
