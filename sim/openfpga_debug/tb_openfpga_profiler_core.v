...
module tb_openfpga_profiler_core;
...
wire metric_ready;
reg [15:0] metric_id = `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT;
...
openfpga_profiler_adapter u_adapter (
    .clk(clk),
    .rst(rst),
    .timestamp(timestamp),
    .snapshot_valid(snapshot_valid),
...
initial begin
    ...
    metric_id = `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT;
    ...
    enable = 1'b1;
    send_metric(`OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, 32'd10, 32'd1, 32'd2, 32'd3, 1'b0);
    send_metric(`OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, 32'd5, 32'd2, 32'd3, 32'd4, 1'b0);
    wait_snapshot();
    check(snapshot_metric_id == `OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, "snapshot metric id mismatch");
    ...
    send_metric(`OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, 32'd100, 32'd0, 32'd0, 32'd0, 1'b0);
    repeat (8) @(posedge clk);
    check(!snapshot_valid, "masked metric should not emit snapshot");
    metric_mask = 32'hFFFFFFFF;
    $display("INFO: metric mask checked");

    send_metric(`OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, 32'd7, 32'd0, 32'd0, 32'd0, 1'b0);
    pulse_clear();
    ...
    snapshot_ready = 1'b0;
    send_metric(`OFD_PROFILER_METRIC_AXIS_DEMO_THROUGHPUT, 32'd1, 32'd1, 32'd1, 32'd1, 1'b0);
    wait_snapshot();
    ...
    alert_ready = 1'b0;
    send_metric(`OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL, 32'd60, 32'd0, 32'd0, 32'd0, 1'b1);
    check(alert_valid, "metric overflow should emit alert");
    check(alert_metric_id == `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL, "alert metric id mismatch");
    check(alert_code == `OFD_PROFILER_ALERT_OVERFLOW, "alert code mismatch");
    wait_snapshot();
    ...
    @(posedge clk);
    #1;
    if (msg_valid && msg_type == `OFD_TYPE_PROFILER_ALERT) begin
        check(payload_len == `OFD_PROFILER_LEN_ALERT, "alert payload length mismatch");
        check(payload_flat[47:32] == `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL, "alert payload metric mismatch");
        check(payload_flat[63:56] == `OFD_PROFILER_ALERT_OVERFLOW, "alert payload code mismatch");
        @(posedge clk);
        #1;
    end
    wait_snapshot();
    ...
    check(msg_type == `OFD_TYPE_PROFILER_SNAPSHOT, "adapter snapshot type mismatch");
    check(payload_len == `OFD_PROFILER_LEN_SNAPSHOT, "snapshot payload length mismatch");
    check(payload_flat[47:32] == `OFD_PROFILER_METRIC_FIFO_DEMO_LEVEL, "snapshot payload metric mismatch");
    check(payload_flat[63:48] & `OFD_PROFILER_FLAG_VALID, "snapshot payload VALID flag mismatch");
    check(payload_flat[239:224] != 16'd0, "snapshot payload overflow count mismatch");

    if (errors == 0) begin
        $display("PASS: OpenFPGA Profiler M18 core snapshot checks passed");
    end else begin
