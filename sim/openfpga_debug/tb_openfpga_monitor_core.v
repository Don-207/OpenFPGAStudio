...
module tb_openfpga_monitor_core;
...
initial begin
    ...
    request(16'd1, `OFD_MON_ADDR_ID, 1'b0, 32'd0, 32'd0);
    check(resp_status == `OFD_MON_STATUS_OK, "MONITOR_ID read should pass");
    check(resp_rdata == `OFD_MONITOR_ID_VALUE, "MONITOR_ID value mismatch");

    request(16'd2, `OFD_MON_ADDR_LED_CONTROL, 1'b1, 32'h0000000F, 32'h0000000F);
    check(resp_status == `OFD_MON_STATUS_OK, "LED_CONTROL write should pass");
    check(led_control == 32'h0000000F, "LED_CONTROL should update");
    request(16'd3, `OFD_MON_ADDR_LED_CONTROL, 1'b1, 32'h00000005, 32'h0000000A);
    ...
    request(16'd4, `OFD_MON_ADDR_ID, 1'b1, 32'hFFFFFFFF, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_DENIED, "write to RO should be denied");

    dut.u_reg_bank.error_status = 32'h0000000F;
    request(16'd5, `OFD_MON_ADDR_ERROR_STATUS, 1'b1, 32'h00000005, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_OK, "W1C write should pass");
    check(error_status == 32'h0000000A, "W1C should clear written one bits");

    request(16'd6, `OFD_MON_ADDR_CLEAR_COUNTERS, 1'b1, 32'h00000001, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_OK, "TRIGGER write should pass");
    ...
    check(resp_status == `OFD_MON_STATUS_BAD_ADDR, "bad address should be rejected");

    request(16'd8, `OFD_MON_ADDR_DEMO_PERIOD, 1'b1, 32'd0, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_BAD_VALUE, "zero demo period should be rejected");

    request(16'd9, `OFD_MON_ADDR_PROFILER_ID, 1'b0, 32'd0, 32'd0);
    check(resp_status == `OFD_MON_STATUS_OK, "PROFILER_ID read should pass");
    check(resp_rdata == `OFD_PROFILER_ID_VALUE, "PROFILER_ID value mismatch");

    request(16'd10, `OFD_MON_ADDR_PROFILER_CONTROL, 1'b1, 32'h00000001, 32'h00000001);
    check(resp_status == `OFD_MON_STATUS_OK, "PROFILER_CONTROL write should pass");
    check(profiler_control[0], "PROFILER_CONTROL enable should update");

    request(16'd11, `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD, 1'b1, 32'd12, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_OK, "PROFILER_SAMPLE_PERIOD write should pass");
    check(profiler_sample_period == 32'd12, "PROFILER_SAMPLE_PERIOD should update");

    request(16'd12, `OFD_MON_ADDR_PROFILER_SAMPLE_PERIOD, 1'b1, 32'd0, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_BAD_VALUE, "zero profiler period should be rejected");

    request(16'd13, `OFD_MON_ADDR_PROFILER_CLEAR, 1'b1, 32'h00000001, 32'hFFFFFFFF);
    check(resp_status == `OFD_MON_STATUS_OK, "PROFILER_CLEAR trigger should pass");
    check(saw_profiler_clear_pulse, "PROFILER_CLEAR should pulse for one cycle");

    if (errors == 0) begin
        $display("PASS: OpenFPGA Monitor M14 core register checks passed");
    end else begin
