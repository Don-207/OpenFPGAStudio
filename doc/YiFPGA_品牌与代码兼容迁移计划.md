...
1 个 ILA core，脚本输出 `PASS`。直接 FTDI/USER2 在 10 MHz TCK、1 MiB
窗口下对 256/512/1,024 B block 分别实测 `118,886.8`、`197,819.8` 和
`241,204.1 B/s`，确认底层链路和 ILA 共存镜像未造成性能回归。初始
Bridge 复测只达到 72,573.3 B/s，定位为每个 mailbox block 将 header、payload
和 commit 分成 3 次 worker 调度的 Host 开销；将完整 block 事务合并为单次
worker 调度后，Bridge 在 10 MHz、1,024 B 下运行 60.001 s，持续接收
16,257,024 B（`270,944.099 B/s`），P50/P99 data gap 为 `2.043/9.837 ms`，
...
- 不伪造已有`v1.0.0`标签；是否为历史 baseline 补标签必须单独确认，
  品牌迁移使用新的 minor 版本标签。
