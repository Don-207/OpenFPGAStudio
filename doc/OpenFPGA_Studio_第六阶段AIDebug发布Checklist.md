...
- [x] Transport 或离线派生损坏场景绑定同一次脱敏板级原始捕获。
- [x] LA 缺 chunk 派生场景绑定同一次脱敏板级原始捕获。
- [x] 每个板级 snapshot schema/hash 通过，规则命中且引用有效。
- [x] 每个注入场景恢复到记录的正常 baseline。
- [x] 完成不少于 30 分钟持续接收与周期诊断并记录数据量、drop、内存和取消次数。
- [x] AI Disabled 条件下复测 UART/JTAG 数据面，UART checksum/version error 为 0。
- [x] 发布负责人复核已知限制并签署第六阶段板级发布（2026-07-17）。

发布结论：自动门禁、五类板级场景证据绑定、恢复验证、组合长稳及发布负责人签署全部完成；第六阶段 AI Debug 板级发布通过。
