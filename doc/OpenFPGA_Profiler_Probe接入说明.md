...
`openfpga_profiler_axis_probe` observes `valid/ready/keep/last`.
...
`openfpga_profiler_fifo_probe` tracks a clear-delimited FIFO window.
...
`openfpga_profiler_frame_probe` tracks completed, dropped, and errored frames.
...
`openfpga_profiler_latency` supports one outstanding transaction.
