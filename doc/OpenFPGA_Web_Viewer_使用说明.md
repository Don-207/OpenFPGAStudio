...
`Inject Sample` 会额外注入四类 `PROFILER_SNAPSHOT` 和一条 `PROFILER_ALERT`，用于无硬件验收 Profiler 视图、JSONL 导出和 `window.openfpgaViewerTest.injectProfilerSample()` 测试钩子。
...
# 或设置 OPENFPGA_VIEWER_BROWSER=/path/to/chromium
```
